...
DirTree_c::DirTree_c ()
	...
	, m_sPath			( L"\\" )
{
	m_Sorter.SetVisibility ( g_tConfig.file_pane_show_hidden, g_tConfig.file_pane_show_system, g_tConfig.file_pane_show_rom );
}
...
void DirTree_c::PopulateTreeLevel ( const Str_c & sPath, HTREEITEM hParent )
{
	...
	for ( int i = 0; i < m_Sorter.GetNumFiles (); ++i )
	{
		const FileInfo_t & tInfo = m_Sorter.GetFile ( i );
		if ( tInfo.m_tData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY && ! ( tInfo.m_uFlags & FLAG_PREV_DIR ) )
		{
			TreeItemInfo_t * pItemInfo =  new TreeItemInfo_t;
			m_dAllocated.Add ( pItemInfo );
			pItemInfo->m_sPath = sPath;
			AppendSlash ( pItemInfo->m_sPath );
			pItemInfo->m_sPath = pItemInfo->m_sPath + tInfo.m_tData.cFileName;
        
			int iIcon = m_Sorter.GetFileIconIndex ( i );

			tInsert.hInsertAfter = hInsertAfter;
			tInsert.item.iImage = iIcon;
			tInsert.item.iSelectedImage = iIcon;
			tInsert.item.pszText = (wchar_t *)tInfo.m_tData.cFileName;
			tInsert.item.lParam = (LPARAM) pItemInfo;
			tInsert.item.cChildren = HasSubDirs ( pItemInfo->m_sPath ) ? 1 : 0;
...
void FileTree_c::OnGetDispInfo ( LV_DISPINFO * pInfo )
{
	...
	pInfo->item.iImage = m_Sorter.GetFileIconIndex ( pInfo->item.lParam );
}
...
void FileTree_c::OnItemActivate ( NMLISTVIEW * pInfo )
{
	...
	{
		const FileInfo_t & Info = m_Sorter.GetFile ( Item.lParam );
		if ( Info.m_tData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY )
			ExpandPath ( m_Sorter.GetDirectory () + Info.m_tData.cFileName );
	}
}
...
void FileTree_c::PopulateList ()
{
	...
	for ( int i = 0; i < m_Sorter.GetNumFiles (); ++i )
	{
		const FileInfo_t & File = m_Sorter.GetFile ( i );
		
		if ( File.m_uFlags & FLAG_PREV_DIR )
			continue;

		if ( ( File.m_tData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY ) && ! m_bAcceptDirs )
			continue;

		if ( ! m_Filter.Fits ( File.m_tData.cFileName ) )
			continue;

		Item.iItem = nAdded++;
		Item.pszText = (wchar_t *)m_Sorter.GetFile ( i ).m_tData.cFileName;
		Item.lParam = i;
...
protected:
	...
	virtual void OnCommand ( int iItem, int iNotify )
	{
		...
		case IDOK:
			...
			{
				if ( m_FileTree.HaveSelectedDir () )
				{
					m_sPath = m_FileTree.Path ();
					Close ( iItem );
				}
				else
					ShowErrorDialog ( m_hWnd, Txt ( T_MSG_ERROR ), Txt ( T_DLG_TREE_NOFILE ), IDD_ERROR_OK );
			}
			break;
		case IDCANCEL:
			Close ( iItem );
...
bool ShowFileTreeDlg ( HWND hParent, Str_c & sPath, bool bAcceptFolders, const TreeFilters_t * dFilters )
{
