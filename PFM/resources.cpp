...
{
	LOGFONT tFont;
	tFont.lfHeight			= cfg->font_height;
	tFont.lfWidth			= 0;
	tFont.lfEscapement		= 0;
	tFont.lfOrientation		= 0;
	tFont.lfWeight			= cfg->font_bold ? FW_BOLD : FW_NORMAL;
	...
	tFont.lfQuality			= cfg->font_clear_type ? CLEARTYPE_QUALITY : DEFAULT_QUALITY;
	tFont.lfPitchAndFamily	= DEFAULT_PITCH;
	wcscpy ( tFont.lfFaceName, cfg->font_name );
...
{
	return m_tMetric.tmHeight ? m_tMetric.tmHeight : cfg->font_height;
}
...
int PanelFont_c::GetWidth ( wchar_t cChar ) const
{
	ABC tABC;
	HDC hDC = g_tResources.m_hBitmapDC;
	GetCharABCWidths ( hDC, cChar, cChar, &tABC );
	return tABC.abcA + tABC.abcB + tABC.abcC;
}
...
{
	m_hActiveHdrBrush	= CreateSolidBrush ( clrs->pane_header_active );
	m_hPassiveHdrBrush	= CreateSolidBrush ( clrs->pane_header_inactive );
	m_hBackgroundBrush	= CreateSolidBrush ( clrs->pane_back );
	m_hCursorBrush		= CreateSolidBrush ( clrs->pane_cursor_bar );
	m_hCursorBrushIA	= CreateSolidBrush ( clrs->pane_cursor_bar_inactive );

	m_hBorderPen	= CreatePen ( PS_SOLID, 1, clrs->pane_separators );
	m_hOverflowPen	= CreatePen ( PS_SOLID, 1, clrs->pane_overflow );
	...
	m_hBmpSelect = (HBITMAP) LoadImage ( ResourceInstance (), MAKEINTRESOURCE ( IsVGAScreen () ? IDB_SELECT_VGA : IDB_SELECT ), IMAGE_BITMAP, 0, 0, 0 );

	BITMAP tBmp;
	GetObject ( m_hBmpBtnMax, sizeof ( BITMAP ), (void *) &tBmp);
	m_iTopIconSize = tBmp.bmWidth;
...
HBITMAP ReadTGA ( HMODULE hResInstance, DWORD uResource, SIZE * pSize )
{
	HRSRC hRSrc = FindResource ( hResInstance, MAKEINTRESOURCE (uResource), RT_RCDATA );
	if ( !hRSrc )
		return NULL;

	DWORD uSize = SizeofResource ( hResInstance, hRSrc );
	if ( !hRSrc )
		return NULL;

	HGLOBAL hResData = LoadResource ( hResInstance, hRSrc );
	if ( !hResData )
		return NULL;
	...
	if ( TGAReadImage ( (BYTE*)pData, uSize, TGA ) )
	{
		if ( pSize )
		{
			pSize->cx = TGA.m_Header.width;
			pSize->cy = TGA.m_Header.height;
		}

		hResult = CreateBitmap ( TGA.m_Header.width, TGA.m_Header.height, 1, TGA.m_Header.depth, TGA.m_pImgData );
		TGAFreeData ( TGA );
	}
