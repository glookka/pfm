...
private:
	...
	void Browse ()
	{
		...
		if ( FileTreeDlg ( m_hWnd, sFile, false, &dFilters ) )
		{
			// add to apps
...
void OpenWithDlg ( const Str_c & sFileName )
{
	...
	{
		int iNewApp = OpenWithDlg.m_iNewApp;
		Assert ( iNewApp != -1 );
		...
		tExecuteInfo.lpParameters= OpenWithDlg.m_bQuotes ? sNameToExec : sFileName;
		tExecuteInfo.nShow		 = SW_SHOW;
		tExecuteInfo.hInstApp	 = g_hAppInstance;
		ShellExecuteEx ( &tExecuteInfo );

		g_tRecent.AddOpenWith ( GetExt ( sFileName ), tAppInfo.m_sName, tAppInfo.m_sFileName, OpenWithDlg.m_bQuotes );

		if ( OpenWithDlg.m_bAlways )
			apps::Associate ( sFileName, iNewApp, OpenWithDlg.m_bQuotes );
	}
}
