...
bool HandleSelectionMenu ( int iCommand )
{
	...
	case IDM_SELECTION_SELECT_NONE:
		pPanel->MarkAllFiles ( false, true );
		return true;
...
bool OnCommand ( int iCommand )
{
	...
	case IDC_TOOLBAR:
		ShowSecondToolbar ( ! cfg->secondbar );
		FMSetFullscreen ( cfg->fullscreen );
		FMRepositionPanels ( true );
		return true;

	case IDC_MISC:
		{
			CheckMenuItem ( g_hMiscMenu, IDM_MISC_FULLSCREEN, MF_BYCOMMAND | ( cfg->fullscreen ? MF_CHECKED : MF_UNCHECKED ) );
...
bool OnInitMenuPopup ( HMENU hMenu )
{
	if ( hMenu == g_hSK1Menu )
	{
		bool bToolbarHidden = cfg->wm5_toolbar && cfg->wm5_hide_toolbar;
		...
		CheckMenuItem ( hMenu, IDM_MISC_FULLSCREEN, MF_BYCOMMAND | ( cfg->fullscreen ? MF_CHECKED : MF_UNCHECKED ) );
		return true;
	}

	if ( hMenu == g_hSK2Menu )
	{
		CheckMenuItem ( hMenu, IDC_FASTNAV, MF_BYCOMMAND | ( cfg->fastnav ? MF_CHECKED : MF_UNCHECKED ) );
		...
		Panel_c * pPanel = FMGetActivePanel ();
		HANDLE hPlugin = pPanel->GetActivePlugin ();
		bool bPlugin = hPlugin != INVALID_HANDLE_VALUE;
		bool bHandlesRealNames = bPlugin && g_PluginManager.HandlesRealNames ( hPlugin );
		bool bOneFile = !pPanel->GetNumMarked ();
		bool bDir = false;
		if ( pPanel->GetNumItems () > 0 )
			bDir = !!(pPanel->GetItem ( pPanel->GetCursorItem () ).m_FindData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY);

		// file menu enable/disable stuff
		EnableMenuItem ( hMenu, IDC_COPYMOVE_FILES, ( !bPlugin || bHandlesRealNames ) ? MF_ENABLED : MF_GRAYED );
		EnableMenuItem ( hMenu, IDC_RENAME, ( !bPlugin || bHandlesRealNames ) ? MF_ENABLED : MF_GRAYED );
		EnableMenuItem ( hMenu, IDC_DELETE_FILES, ( !bPlugin || bHandlesRealNames || bPlugin && g_PluginManager.HandlesDelete ( hPlugin ) ) ? MF_ENABLED : MF_GRAYED );
		EnableMenuItem ( hMenu, IDC_MKDIR, ( !bPlugin || bHandlesRealNames ) ? MF_ENABLED : MF_GRAYED );
		// new menu: fixme
		EnableMenuItem ( hMenu, IDC_FIND, ( !bPlugin || bPlugin && g_PluginManager.HandlesFind ( hPlugin ) ) ? MF_ENABLED : MF_GRAYED );
		EnableMenuItem ( hMenu, IDC_VIEW, ( bOneFile && !bDir && ( !bPlugin || bHandlesRealNames ) ) ? MF_ENABLED : MF_GRAYED );
		EnableMenuItem ( hMenu, IDC_SHORTCUT, ( !bPlugin || bHandlesRealNames ) ? MF_ENABLED : MF_GRAYED );
		EnableMenuItem ( hMenu, IDC_PROPERTIES, ( !bPlugin || bHandlesRealNames ) ? MF_ENABLED : MF_GRAYED );
...
void ShowSecondToolbar ( bool bShow )
{
	...
	SendMessage ( g_hMenuBar, TB_SETSTATE, IDC_TOOLBAR, ( cfg->secondbar ? TBSTATE_CHECKED : 0 ) | TBSTATE_ENABLED );
}
...
void UpdateToolbar ()
{
	...
	bool bHaveFiles  = ! ( nFiles == 0 || ( nFiles == 1 && ( pPanel->GetFile ( 0 ).m_uFlags & FILE_PREV_DIR ) ) );
...
HWND CreateMenuBar ()
{
	...
	if ( GetOSVersion () == WINCE_50 && cfg->wm5_toolbar )
	{
	...
	else
	{
		...
		SendMessage ( hBar, TB_SETSTATE, IDC_FASTNAV, ( cfg->fastnav ? TBSTATE_CHECKED : 0 ) | TBSTATE_ENABLED );
		SendMessage ( hBar, TB_SETSTATE, IDC_TOOLBAR, ( cfg->secondbar ? TBSTATE_CHECKED : 0 ) | TBSTATE_ENABLED );
...
{
	return cfg->secondbar;
}

bool IsToolbarHidden ()
{
	return cfg->wm5_toolbar && cfg->wm5_hide_toolbar;
}
